...
### Atualização 2026-06-30 11:20 UTC
...
**Erros explícitos de delivery do scheduler:** 0. **Falhas em stdout de job:** não avaliadas por stdout bruto; somente campos estruturados seguros foram usados.

**Jobs ativos sem primeira execução registrada:** 2.
- `27916e815136` Skill Surface Diet monthly heavy-skill curation audit — active, `deliver=origin`, sem Last run; próxima execução: 2026-07-01T06:15:00+00:00.
- `dbafe9b8bfca` Skill Surface Diet weekly supervised review prompt — active, `deliver=origin`, sem Last run; próxima execução: 2026-07-06T05:45:00+00:00.
...
**Drift/notable reconciliation:**
- Contagem mudou vs 2026-06-29: 46 → 50 totais; 42 → 46 ativos; 4 pausados estável.
- Novos/observados vs snapshot anterior: `ce165b7246d3` Skill Surface Diet daily drift audit read-only — active/`deliver=origin`/ok; `dbafe9b8bfca` Skill Surface Diet weekly supervised review prompt — active/`deliver=origin`/sem primeira execução; `27916e815136` Skill Surface Diet monthly heavy-skill curation audit — active/`deliver=origin`/sem primeira execução; `ec0473a3a010` Hermes Brain safe GitHub checkpoint 30min — active/local/ok.
- `dbafe9b8bfca` e `27916e815136` precisam ser acompanhados até a primeira execução registrada.
- Jobs Skill Surface Diet com `deliver=origin` precisam de reconciliação documental do owner/finalidade/critério de sucesso/contrato de alerta para evitar ruído em Telegram.
- `fac3e13782c4` segue `deliver=origin` apesar de declarar silent-OK; permanece gap documental de contrato de alerta/ruído.
- `2e5bc91d27d6` Hermes Nightly Operations Audit OS — 02h50 BRT segue ativo/local/ok e ainda merece reconciliação documental de owner/finalidade/critério de sucesso em rodada própria.
- Documentação que descreve Mordomo/LK Growth/SPITI gateway watchdogs como ativos deve permanecer marcada como histórica até nova evidência viva mostrar reativação.
...
**Relatório completo:** `reports/governance/runtime-truth-reconciler-2026-06-30.md`

---

### Atualização 2026-06-29 11:20 UTC

**Fonte:** `cronjob list` solicitado; `cronjob` não disponível no PATH deste runtime. Fallback canônico usado: `HERMES_HOME=/opt/data /opt/hermes/.venv/bin/hermes cron list --all`. Fonte estruturada sanitizada para contagem: `/opt/data/cron/jobs.json`.

**Contagem:** 46 jobs totais — 42 ativos, 4 pausados.
...
- Novos/observados vs snapshot anterior: `e3af978c6af6` Honcho Nightly Intelligence Enforcement Audit — 02h35 BRT, active/local/ok; `ba8ca37bfebd` Honcho semantic contamination audit — daily local, active/local/ok; `fac3e13782c4` Honcho quality watchdog — silent OK, active/`deliver=origin`/ok.
- Job futuro anterior ausente na evidência atual: `00a9f839879f` Honcho utility go/no-go review one-shot não aparece mais em `/opt/data/cron/jobs.json`; tratar como histórico/removido ou executado/limpo fora deste snapshot, sem manter como ativo sem evidência viva.
- `fac3e13782c4` usa `deliver=origin` apesar de declarar silent-OK; precisa reconciliação documental do contrato de alerta/ruído antes de qualquer mudança operacional.
- `2e5bc91d27d6` Hermes Nightly Operations Audit OS — 02h50 BRT segue ativo/local/ok e ainda merece reconciliação documental de owner/finalidade/critério de sucesso em rodada própria.
- Documentação que descreve Mordomo/LK Growth/SPITI gateway watchdogs como ativos deve permanecer marcada como histórica até nova evidência viva mostrar reativação.

**Jobs deliver=origin (saídas intencionais/condicionais):** Ativos: Mesa COO diária Telegram (`749ee30b51eb`), Relatório Hermes diário 01h+02h+02h15+02h25+02h50 + Score 0–100 — 03h Telegram (`98478b820720`), Reminder OS — 2h open-loop watchdog (`518634d5ea60`), Honcho quality watchdog — silent OK (`fac3e13782c4`). Pausados: LK specialist Telegram gateway watchdog (`955dc769b5a6`).

**Relatório completo:** `reports/governance/runtime-truth-reconciler-2026-06-29.md`

---

### Atualização 2026-06-28 11:20 UTC

**Fonte:** `cronjob list` solicitado; `cronjob` não disponível no PATH deste runtime. Fallback canônico usado: `HERMES_HOME=/opt/data /opt/hermes/.venv/bin/hermes cron list --all`. Fonte estruturada sanitizada para contagem: `/opt/data/cron/jobs.json`.

**Contagem:** 44 jobs totais — 40 ativos, 4 pausados.
...
**Drift/notable reconciliation:**
- Contagem estável vs 2026-06-27: 44 totais / 40 ativos / 4 pausados.
- Drift positivo: `e3279babbc4a` LK 09h previous-day sales report external delivery recuperou de `error` para `ok` na evidência viva atual.
...
**Jobs deliver=origin (saídas intencionais/condicionais):** Ativos: Mesa COO diária Telegram (`749ee30b51eb`), Relatório Hermes diário 01h+02h+02h15+02h25+02h50 + Score 0–100 — 03h Telegram (`98478b820720`), Reminder OS — 2h open-loop watchdog (`518634d5ea60`), Honcho utility go/no-go review one-shot (`00a9f839879f`). Pausados: LK specialist Telegram gateway watchdog (`955dc769b5a6`).

**Relatório completo:** `reports/governance/runtime-truth-reconciler-2026-06-28.md`

---

### Atualização 2026-06-27 11:20 UTC
...
**`last_status` não-ok:** 1.
- `e3279babbc4a` LK 09h previous-day sales report external delivery — active, `deliver=local`, erro de script por payload gerado sem `message/html/email_meta`; último run: 2026-06-26T12:00:58.418039+00:00.
...
- Drift negativo: `e3279babbc4a` passou de `ok` no snapshot de 2026-06-26 para `error`; o erro aponta payload gerado sem `message/html/email_meta` no envio externo aprovado da rotina LK 09h.
- `00a9f839879f` segue ativo/`deliver=origin`/sem primeira execução; acompanhar até 2026-06-29T01:10 UTC e documentar owner/finalidade/critério de sucesso/contrato de alerta.
- `2e5bc91d27d6` Hermes Nightly Operations Audit OS — 02h50 BRT segue ativo/local/ok e ainda merece reconciliação documental de owner/finalidade/critério de sucesso em rodada própria.
- Documentação que descreve Mordomo/LK Growth/SPITI gateway watchdogs como ativos deve permanecer marcada como histórica até nova evidência viva mostrar reativação.
...
**Relatório completo:** `reports/governance/runtime-truth-reconciler-2026-06-27.md`

---

### Atualização 2026-06-26 11:21 UTC
...
**`last_status` não-ok:** 1.
- `bc96bb03d2b0` Hermes Memory OS daytime checker/router — 30min alert-only — active, `deliver=local`, erro por timeout interno de 240s ao rodar o checker Memory OS diurno; último run: 2026-06-26T11:01:52.486192+00:00.

**Erros explícitos de delivery do scheduler:** 0. **Falhas em stdout de job:** não avaliadas por stdout bruto; somente campos estruturados seguros foram usados.

**Jobs ativos sem primeira execução registrada:** 1.
- `00a9f839879f` Honcho utility go/no-go review one-shot — active, `deliver=origin`, sem Last run / sem Last run; próxima execução: 2026-06-29T01:10:16+00:00.
...
**Relatório completo:** `reports/governance/runtime-truth-reconciler-2026-06-26.md`

---

### Atualização 2026-06-25 11:20 UTC
...
**Drift/notable reconciliation:**
- Contagem mudou vs 2026-06-24: 43 → 44 totais; 39 → 40 ativos; 4 → 4 pausados.
- Novo job vivo vs snapshot anterior: `c933dd2e30ec` WACLI health read-only pre-Zipper 08h50 BRT — active, `deliver=local`, ok / 2026-06-24T14:59:33.603762+00:00.
- Drift positivo: `357d40a5863e` e `e2f169cc046a`, que estavam com `last_status=error` em 2026-06-24, agora constam `ok` na evidência viva.
- `00a9f839879f` segue ativo/`deliver=origin`/sem primeira execução; acompanhar até 2026-06-29T01:10 UTC e documentar owner/finalidade/critério de sucesso/contrato de alerta.
- Jobs Honcho `7d32b8b77317`, `39b176e08174` e `16dfc4d14c85` seguem vivos/ok com `deliver=local`; manter reconciliação documental da mudança de entrega para evitar falso gap de ruído.
- `2e5bc91d27d6` Hermes Nightly Operations Audit OS — 02h50 BRT segue ativo/local/ok e ainda merece reconciliação documental de owner/finalidade/critério de sucesso em rodada própria.
- Documentação que descreve Mordomo/LK Growth/SPITI gateway watchdogs como ativos deve permanecer marcada como histórica até nova evidência viva mostrar reativação.
...
**Relatório completo:** `reports/governance/runtime-truth-reconciler-2026-06-25.md`

---

### Atualização 2026-06-24 11:21 UTC

**Fonte:** `cronjob list` solicitado; `cronjob` não disponível no PATH deste runtime. Fallback canônico usado: `HERMES_HOME=/opt/data /opt/hermes/.venv/bin/hermes cron list --all`. Fonte estruturada sanitizada para contagem: `/opt/data/cron/jobs.json`.

**Contagem:** 43 jobs totais — 39 ativos, 4 pausados.

**`last_status` não-ok:** 2.
- `357d40a5863e` Zipper OS vendas 09h WhatsApp/email — active, `deliver=local`, erro de script ao interpretar resposta JSON do envio interno aprovado; último run: 2026-06-23T12:00:47.203156+00:00.
- `e2f169cc046a` LK POS Shopify→fila reconciliador — active, `deliver=local`, erro de rede (`Network is unreachable`) no reconciliador Shopify→fila; último run: 2026-06-24T11:21:05.156519+00:00.

**Erros explícitos de delivery do scheduler:** 0 na listagem/estado. **Falhas em stdout de job:** 2 jobs com `last_status=error`; stdout bruto não versionado.
...
**Drift/notable reconciliation:**
- Contagem estável vs 2026-06-23: 43 totais / 39 ativos / 4 pausados.
- Drift negativo de runtime: `357d40a5863e` e `e2f169cc046a` estavam `ok` no relatório de 2026-06-23 e agora aparecem com `last_status=error`.
...
**Jobs deliver=origin (saídas intencionais/condicionais):** Mesa COO diária Telegram (`749ee30b51eb`), Relatório Hermes diário 01h+02h+02h15+02h25+02h50 + Score 0–100 — 03h Telegram (`98478b820720`), Reminder OS — 2h open-loop watchdog (`518634d5ea60`), Honcho utility go/no-go review one-shot (`00a9f839879f`). Pausado com `origin`: LK specialist Telegram gateway watchdog (`955dc769b5a6`).

**Relatório completo:** `reports/governance/runtime-truth-reconciler-2026-06-24.md`

---

### Atualização 2026-06-23 11:21 UTC
...
**Erros explícitos de delivery do scheduler:** 0 na listagem/estado. **Falhas em stdout de job:** não avaliadas por stdout bruto; somente campos estruturados seguros foram usados.
...
**Relatório completo:** `reports/governance/runtime-truth-reconciler-2026-06-23.md`

---

### Atualização 2026-06-22 11:23 UTC
...
**Erros explícitos de delivery do scheduler:** 0 na listagem/estado. **Falhas em stdout de job:** 0 observadas pela listagem resumida.
...
- Novo job vivo: `00a9f839879f` Honcho utility go/no-go review one-shot — active, `deliver=origin`, sem Last run / sem Last run.
- Jobs Honcho `7d32b8b77317`, `39b176e08174` e `16dfc4d14c85` seguem ativos/ok e agora aparecem `deliver=local`, diferente do `deliver=origin` documentado em 2026-06-21; tratar como drift documental/positivo de ruído, não falha.
- `2e5bc91d27d6` Hermes Nightly Operations Audit OS — 02h50 BRT segue ativo/local/ok e ainda merece reconciliação documental de owner/finalidade/critério de sucesso em rodada própria.
- Documentação que descreve Mordomo/LK Growth/SPITI gateway watchdogs como ativos deve permanecer marcada como histórica até nova evidência viva mostrar reativação.

**Jobs deliver=origin (saídas intencionais/condicionais):** Mesa COO diária Telegram (`749ee30b51eb`), Relatório Hermes diário 01h+02h+02h15+02h25+02h50 + Score 0–100 — 03h Telegram (`98478b820720`), Reminder OS — 2h open-loop watchdog (`518634d5ea60`), Honcho utility go/no-go review one-shot (`00a9f839879f`). `LK specialist Telegram gateway watchdog` (`955dc769b5a6`) também está configurado como `origin`, mas está pausado.

**Relatório completo:** `reports/governance/runtime-truth-reconciler-2026-06-22.md`
...
### Atualização 2026-06-20 11:20 UTC
...
**Drift/notable reconciliation:**
- Contagem mudou vs 2026-06-19: 39 → 42 totais; 35 → 38 ativos; 4 pausados estável.
- Novos jobs vivos na evidência, todos ativos/`deliver=origin`/`ok`: `7d32b8b77317` Honcho Hermes memory watchdog silent-OK, `39b176e08174` Honcho memory quality auditor silent-OK, `16dfc4d14c85` Honcho Intelligence Layer v1 weekly silent-OK.
- Nenhum `last_status` não-ok, erro explícito de delivery ou job ativo sem primeira execução registrada na evidência atual.
- `2e5bc91d27d6` Hermes Nightly Operations Audit OS — 02h50 BRT segue ativo/local/ok e ainda merece reconciliação documental de owner/finalidade/critério de sucesso em rodada própria.
- Rotinas previamente problemáticas (`d03fa04e1188`, `c3bb587519d2`, `e3279babbc4a`, `a2ead305eab2`, `357d40a5863e`, `787134d4ac5c`, `a1d1e36f8075`) seguem `ok`; não manter como falha ativa sem nova evidência.
- Documentação que descreve Mordomo/LK Growth/SPITI gateway watchdogs como ativos deve permanecer marcada como histórica até nova evidência viva mostrar reativação.
...
**Relatório completo:** `reports/governance/runtime-truth-reconciler-2026-06-20.md`

---

### Atualização 2026-06-19 11:21 UTC

**Fonte:** `cronjob list` solicitado; `cronjob` não disponível no PATH deste runtime. Fallback canônico usado: `HERMES_HOME=/opt/data /opt/hermes/.venv/bin/hermes cron list --all`.

**Contagem:** 39 jobs totais — 35 ativos, 4 pausados.
...
**Drift/notable reconciliation:**
- Contagem mudou vs 2026-06-18: 40 → 39 totais; 36 → 35 ativos; 4 pausados estável.
- Nenhum `last_status` não-ok, erro explícito de delivery ou job ativo sem primeira execução registrada na evidência atual.
- O relatório de 2026-06-18 não versiona a listagem bruta completa dos 40 jobs; portanto esta execução não afirma com segurança qual job saiu além do histórico já registrado de que `7ef586b9ec1a` não aparece na evidência viva desde 2026-06-18.
- `2e5bc91d27d6` Hermes Nightly Operations Audit OS — 02h50 BRT segue ativo/local/ok e ainda merece reconciliação documental de owner/finalidade/critério de sucesso em rodada própria.
- Rotinas previamente problemáticas (`d03fa04e1188`, `c3bb587519d2`, `e3279babbc4a`, `a2ead305eab2`, `357d40a5863e`, `787134d4ac5c`, `a1d1e36f8075`) seguem `ok`; não manter como falha ativa sem nova evidência.
- Documentação que descreve Mordomo/LK Growth/SPITI gateway watchdogs como ativos deve permanecer marcada como histórica até nova evidência viva mostrar reativação.

**Jobs deliver=origin (saídas intencionais/condicionais):** Mesa COO diária (`749ee30b51eb`), Relatório Hermes 01h+02h+02h15+02h25+02h50 + Score 0–100 — 03h Telegram (`98478b820720`) e Reminder OS — 2h open-loop watchdog (`518634d5ea60`). `LK specialist Telegram gateway watchdog` (`955dc769b5a6`) também está configurado como `origin`, mas está pausado.

**Relatório completo:** `reports/governance/runtime-truth-reconciler-2026-06-19.md`

---

### Atualização 2026-06-18 11:21 UTC
...
**Drift/notable reconciliation:**
- Contagem estável vs 2026-06-17: 40 totais / 36 ativos / 4 pausados.
- Nenhum `last_status` não-ok, erro explícito de delivery ou job ativo sem primeira execução registrada na evidência atual.
- `7ef586b9ec1a` Reanalisar backlog Auto-Remediation em 72h não aparece mais na evidência viva; em 2026-06-17 era o único job ativo sem primeira execução. Tratar como histórico/removido ou executado/limpo fora deste snapshot; não manter como pendência ativa sem nova evidência viva.
- `2e5bc91d27d6` Hermes Nightly Operations Audit OS — 02h50 BRT segue ativo/local/ok e ainda merece reconciliação documental de owner/finalidade/critério de sucesso em rodada própria.
- Rotinas previamente problemáticas (`d03fa04e1188`, `c3bb587519d2`, `e3279babbc4a`, `a2ead305eab2`, `357d40a5863e`, `787134d4ac5c`, `a1d1e36f8075`) seguem `ok`; não manter como falha ativa sem nova evidência.
- Documentação que descreve Mordomo/LK Growth/SPITI gateway watchdogs como ativos deve permanecer marcada como histórica até nova evidência viva mostrar reativação.
...
**Relatório completo:** `reports/governance/runtime-truth-reconciler-2026-06-18.md`
