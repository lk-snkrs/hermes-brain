...
Os documentos `LKGOC-*` foram migrados fisicamente para `areas/lk/sub-areas/collection-optimizer/`. Arquivos remanescentes em `collection-optimizer/LKGOC-*` são apenas redirects legados, não fonte primária. Quando houver conflito:
...
Os arquivos `/opt/data/hermes_bruno_ingest/hermes-brain/areas/lk/sub-areas/collection-optimizer/LKGOC-*` foram substituídos por stubs de redirect para preservar links/workers antigos. Novas leituras e writes canônicos devem usar `collection-optimizer/LKGOC-*`.


Nota: `growth/LKGOC-*` são redirects legados; não usar como fonte primária.
