...
## CORREÇÃO CANÔNICA — status pós-audit 20260606T165945Z

Status atualizado: `DEV_BUILD_ALLOWED / PRODUCTION_BLOCKED`

Correção Lucas:

- Este Contract Lock **não bloqueia write em DEV**.
- Puma Speedcat pode e deve ser reconstruída no tema DEV/unpublished para preview e QA.
- As imagens principais devem ser retiradas/buscadas nos principais veículos de moda/editoriais relevantes, como Vogue/Vogue Brasil, quando forem a melhor referência visual.
- Todo write Shopify deve ser feito sempre no DEV/unpublished e não precisa de autorização prévia.
- Write direto em Production/main é extremamente proibido.
- A aprovação de Lucas será necessária apenas para merge/promoção para Production.

Leitura correta deste contrato:

- DEV: liberado para build/teste/preview.
- Production: bloqueado até approval final.
