...
- Gate Tiny Sync ingress público ativado/validado em 2026-06-11T12:44:28Z após aprovação escopada de Lucas para concluir próximos passos. Serviço `hermes-webhooks` no Vercel deployado e rota pública operacional `https://hermes-webhooks.lucascimino.com/webhooks/lk-stock/tiny/stock?secret=[REDACTED]` validada com probe negativo `401` sem assinatura/secret, probe HMAC válido `200`, probe query-secret válido `200`, readback SQLite local para `TEST-TINY-PUBLIC-HMAC-40` e `TEST-TINY-PUBLIC-QUERY-40`, unittest Stock OS `23 tests OK` e cron registry `e8b35e20751b` ativo/ok. Receipt: `areas/lk/sub-areas/stock/receipts/lk-stock-tiny-sync-runtime-activated-public-ingress-20260611T124308Z.md`. Guardrails preservados: Tiny write `0`, Shopify write `0`, cliente/fornecedor/campanha `0`, pronta entrega pública `0`, `availability_claim_allowed=0`. Configuração Tiny/Olist precisa ser aplicada no painel da integração (`Configurações > E-commerce > Integrações > integração > Webhook > URL de notificações do estoque`) caso não haja ferramenta/API segura disponível no runtime.
