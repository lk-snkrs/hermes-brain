...
- Gate Tiny Sync runtime preparado em 2026-06-11T11:46:38Z após pesquisa de docs oficiais Tiny/Olist: webhook de atualização de estoque, fila `lista.atualizacoes.estoque` e `produto.obter.estoque`. Decision packet: `areas/lk/sub-areas/stock/approval-packets/lk-stock-tiny-sync-runtime-decision-packet-20260611T114638Z.md`; JSON: `areas/lk/sub-areas/stock/reports/lk-stock-tiny-sync-runtime-decision-packet-20260611T114638Z.json`. Estado inicial: **preparado, não ativado**; rota proposta `POST https://hermes-webhooks.lucascimino.com/webhooks/lk-stock/tiny/stock`; exigia aprovação escopada antes de Vercel/webhook/cron/gateway. Guardrails: Tiny write `0`, Shopify write `0`, writes externos `0`, pronta entrega pública `0`, runtime novo `0`.
- Gate Tiny Sync ingress público ativado/validado em 2026-06-11T12:44:28Z após aprovação escopada de Lucas para concluir próximos passos. Serviço `hermes-webhooks` no Vercel deployado e rota pública de estoque `https://hermes-webhooks.lucascimino.com/webhooks/lk-stock/tiny/stock?secret=[REDACTED]` validada com probe negativo `401` sem assinatura/secret, probe HMAC válido `200`, probe query-secret válido `200`, readback SQLite local para `TEST-TINY-PUBLIC-HMAC-40` e `TEST-TINY-PUBLIC-QUERY-40`, unittest Stock OS `23 tests OK` e cron registry `e8b35e20751b` ativo/ok. Receipt: `areas/lk/sub-areas/stock/receipts/lk-stock-tiny-sync-runtime-activated-public-ingress-20260611T124308Z.md`. Guardrails preservados: Tiny write `0`, Shopify write `0`, cliente/fornecedor/campanha `0`, pronta entrega pública `0`, `availability_claim_allowed=0`. Correção de escopo em 2026-06-11: para Stock OS operacional, o painel Tiny/Olist deve ativar **vendas** e **lançamentos de estoque**, não apenas lançamentos de estoque. Motivo: venda/baixa detecta saída e ruptura; lançamento de estoque detecta entrada/ajuste/retorno. `Pedidos enviados` e `notas fiscais autorizadas` ficam desligados por enquanto, salvo novo caso de uso. Para suportar ambos os eventos no mesmo ingress, foi criada e validada a rota pública canônica `https://hermes-webhooks.lucascimino.com/webhooks/lk-stock/tiny/events?secret=[REDACTED]`, route `lk-stock-tiny-events`, event `tiny_webhook`, script `areas/lk/sub-areas/stock/scripts/lk_stock_tiny_events_processor.py`. Probes públicos válidos em 2026-06-11: payload `inclusao_pedido` → HTTP `200`/`tiny_order_event`/`stock_decrement_executed=0`; payload estoque → HTTP `200`/`tiny_stock_snapshot`. Esta é a URL a usar no painel Tiny/Olist para **vendas** e **lançamentos de estoque**.
- Correção de semântica de estoque implementada em 2026-06-11 após orientação de Lucas: o payload de lançamento/atualização de estoque do Tiny/Olist é tratado apenas como **gatilho de mudança**, não como fonte final de quantidade. Para eventos de estoque na rota canônica `/tiny/events`, o processor exige readback read-only via Tiny API `produto.obter.estoque` no `idProduto` e depósito `LK | CONTROLE ESTOQUE`; a quantidade gravada no ledger/base local vem de `quantity_source=tiny_api_readback`. Se o readback falhar, o evento de estoque é rejeitado para evitar sobrescrever a base com quantidade ambígua. Teste TDD adicionado: webhook com `saldo=3` e readback `9` grava `9`; falha de readback bloqueia update. Verificação: `25 tests OK`; probe público real com `HV8547-200-38`/Tiny `1069546133` ignorou `webhook_quantity=999` e retornou `quantity=2.0` de `tiny_api_readback`. Guardrails: Tiny write `0`, Shopify write `0`, writes externos `0`, pronta entrega pública `0`.
- Decisão operacional de arquitetura em 2026-06-11 após orientação de Lucas: **consulta diária/atendimento interno usa DB local como caminho primário**, não Tiny on-demand. Tiny é lento e fica no caminho de sincronização: full sync de madrugada, webhooks durante o dia e readback pontual apenas quando um webhook de estoque chega ou quando houver falha/reconciliação. O DB local deve carregar `source_observed_at`, `stock_freshness`, `quantity_source` e guardrails para deixar claro quando o saldo veio de full sync, webhook/readback ou cache. Consultas normais respondem pelo DB local; se a base estiver quebrada/stale/sem SKU, o agente não deve prometer disponibilidade e deve sinalizar necessidade de reconciliação, em vez de bater no Tiny a cada pergunta.
- Gate Tiny full sync noturno implementado em 2026-06-11: script local/read-only `areas/lk/sub-areas/stock/scripts/lk_stock_tiny_full_sync.py` clona a DB apontada por `lk_stock_os_current_pointer.json`, processa apenas linhas locais seguras (`identity_resolved_safe=1`, Tiny id único, guardrails públicos zerados), consulta Tiny API `produto.obter.estoque` para o depósito `LK | CONTROLE ESTOQUE`, atualiza `current_local_stock`/tabelas locais existentes com `stock_source='tiny_full_sync'` e `stock_freshness='tiny_full_sync_nightly'`, registra `tiny_full_sync_runs` + `tiny_full_sync_item_ledger`, gera JSON/MD e preserva Tiny write `0`, Shopify write `0`, writes externos `0`, public availability `0`. Wrapper silent-OK criado em `/opt/data/profiles/lk-stock/scripts/lk_stock_tiny_full_sync_nightly.py`; cron `c45da7bb0fcb` agendado para `20 6 * * *` UTC (03:20 BRT), `no_agent=true`, profile `lk-stock`, entregando só alerta acionável em falha/parcial. Verificação: TDD RED/GREEN para sync e pointer update; suíte Stock OS `28 tests OK`; smoke real Tiny read-only `--limit 1` retornou `status=ok`, `rows_updated=1`, item `NB-0058078-34`/Tiny `1069539093`, quantity `0.0`, guardrails `0`. O smoke não atualizou pointer; o cron full sem limit atualiza pointer apenas ao finalizar.
- Fechamento da arquitetura DB-local-primary em 2026-06-11: corrigido `areas/lk/sub-areas/stock/scripts/lk_stock_os_query.py` para o caminho quente de consulta usar `current_local_stock` por padrão mesmo quando `current_stock_scored` existir, resolver paths relativos para o Brain canônico e expor quantidade/proveniência (`stock_source`, `stock_freshness`, `source_observed_at`) no JSON. TDD adicionado em `evaluation/test_lk_stock_os_query.py`: uma DB com `current_stock_scored` vazio e `current_local_stock` populado precisa retornar o SKU pelo DB local. Verificação fresca: `29 tests OK`; consulta real `HV8547-200-38` retornou `current_local_stock`, `stock_quantity_max_observed=2.0`, `stock_source=tiny_webhook_runtime`, `stock_freshness=live_tiny_webhook`, `source_observed_at=2026-06-11T16:07:11Z`, guardrails `public_availability_safe=0` e `availability_claim_allowed=0`; probe público sem secret na rota `/tiny/events` retornou `401`; wrapper full sync smoke `LK_STOCK_TINY_FULL_SYNC_LIMIT=1` saiu `0` e ficou silencioso. Guardrails finais: Tiny write `0`, Shopify write `0`, writes externos `0`, cliente/fornecedor/campanha `0`, pronta entrega pública `0`.
- Correção local/cache de identidade sem SKU em 2026-06-12 após caso `Meia Saint Studio Pima Branco`: o produto Shopify `9228298027230` / variant `48283857780958` / inventory item `50411563319518` estava sem SKU/barcode/metafield, e o produto Tiny também estava com `codigo` vazio. Fallback Tiny read-only por título exato encontrou Tiny id `1071195617`, nome `Meia Saint Studio Pima Branco`, situação `A`, saldo `1.0` no depósito `LK | CONTROLE ESTOQUE`, `desconsiderar=N`. Criada nova DB apontada por `lk_stock_os_current_pointer.json`: `areas/lk/sub-areas/stock/data/lk_stock_os_current_local_identity_aliases_20260612T005354Z.db`, com alias local `SHOPIFYVAR-48283857780958`, status `CONSULTABLE_LOCAL_TITLE_FALLBACK_IDENTITY_UNLINKED`, `local_consult_safe=1`, `identity_resolved_safe=0`, `public_availability_safe=0`, `availability_claim_allowed=0`. O lookup agora encontra o item por variant id `48283857780958`, handle `meia-saint-studio-pima-branco`, Tiny id `1071195617` e metadata `inventory_item_id`/GMC offerId; `lk_stock_os_query.py` passou a buscar também `tiny_codigo`, `tiny_id_candidates` e `data_quality_gap` para produtos sem SKU. Artefatos: `areas/lk/sub-areas/stock/reports/lk-stock-local-identity-alias-meia-saint-studio-pima-branco-20260612T005354Z.json`, `areas/lk/sub-areas/stock/reports/lk-stock-local-identity-alias-meia-saint-studio-pima-branco-20260612T005354Z.csv`, `areas/lk/sub-areas/stock/approval-packets/lk-stock-local-identity-alias-meia-saint-studio-pima-branco-20260612T005354Z.md`. Verificação: consultas por `48283857780958`, `meia-saint-studio-pima-branco` e `1071195617` retornaram 1 linha no `current_local_stock`; suíte Stock OS `30 tests OK`. Guardrails: Tiny write `0`, Shopify write `0`, writes externos `0`, pronta entrega pública `0`.
