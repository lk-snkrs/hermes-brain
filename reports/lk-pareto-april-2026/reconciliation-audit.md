...
A divergência principal está em **receita e-commerce real vs dashboards de plataforma**: agora a auditoria calcula a metodologia da Pareto diretamente das fontes, sem copiar o PDF. O resumo executivo vem do GA4 Data API (`totalRevenue`, `transactions`, `sessions`) e reproduz `233 pedidos`, `R$ 722.636,36`, `166.003 sessões` e ticket `R$ 3.101,44`. A contribuição real por canal vem de GA4 `sessionDefaultChannelGroup`/`sessionSourceMedium`, reproduzindo exemplos como `Paid Social R$ 211.329`, `facebook / paid R$ 181.859,02` e `google / cpc R$ 130.069,50`. Meta Ads e Google Ads permanecem dashboards de plataforma: Meta é calculado via Meta API direto e bate 100%; Google é calculado via Metricool Google Ads e bate spend 100%, com valor atribuído ~98,84% do PDF, aceitável pela tolerância de 99%/atualização de API.
