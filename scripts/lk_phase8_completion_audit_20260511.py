...
Consolidates the final state of LK-AUTO-001..006 after activation/readiness work.
...
def build() -> dict[str, Any]:
    ...
    check('six_automations_tracked', len(op_by_id) == 6, 'fail', 'Exactly 6 LK-AUTO entries must be tracked.', {'count': len(op_by_id)})
    ...
    all_counts = {
        ...
        'production_writes': (ledger.get('summary') or {}).get('production_writes', 0) + (klaviyo.get('summary') or {}).get('production_writes', 0) + (sourcing.get('summary') or {}).get('shopify_writes', 0) + (sourcing.get('summary') or {}).get('tiny_writes', 0) + (sourcing.get('summary') or {}).get('price_or_stock_changes', 0),
    ...
    return {
        ...
        'summary': {
            'automations_tracked': 6,
            'active_crons': 3,
            'mandatory_deliveries': 2,
