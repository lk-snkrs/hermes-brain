...
def collect_inline_image_paths(rep: Dict[str, Any]) -> List[Path]:
    ...
    for row in rep.get('creative_performance') or []:
        ...
        if path.exists() and path.is_file():
            paths.append(path)
    # preserve order, dedupe exact paths
    seen = set()
    out = []
    for path in paths:
...
def build_email_mime(from_addr: str, to_addr: str, subject: str, html_body: str, text_body: str, inline_images: List[Path] | None = None) -> MIMEMultipart | MIMEText:
    ...
    for idx, path in enumerate(inline_images, 1):
...
def send_gmail(secrets: Dict[str,str], to_addr: str, subject: str, html_body: str, text_body: str, inline_images: List[Path] | None = None) -> Dict[str, Any]:
    credential_sets = [
    ...
    msg = build_email_mime(from_addr, to_addr, subject, html_body, text_body, inline_images)
...
def main():
    ...
    ap.add_argument('--allow-send-creative-assets-inline', action='store_true', help='Allow --send with locally harvested creative assets attached as inline CID images. Requires fresh QA and explicit approval.')
    ap.add_argument('--email-mime-preview', default=None, help='Write the exact MIME message that would be sent, without sending. Useful to inspect inline CID images safely.')
    args = ap.parse_args()
    if args.send and args.include_creative_assets and not args.allow_send_creative_assets_inline:
        raise RuntimeError('Creative assets require inline CID attachment workflow: add --allow-send-creative-assets-inline only after fresh QA/approval. Cron remains without creatives by default.')
    ...
    sent = None
    inline_images = collect_inline_image_paths(rep) if args.include_creative_assets and args.allow_send_creative_assets_inline else []
    subject = f'LK — Vendas Influencers semanal ({windows["current"][0].date()} a {windows["current"][1].date()})'
    if args.email_mime_preview:
        preview_to = args.to or secrets.get('GMAIL_USER') or 'preview@example.com'
        preview_from = secrets.get('GMAIL_USER') or preview_to
        mime_msg = build_email_mime(preview_from, preview_to, subject, html_body, md, inline_images)
        Path(args.email_mime_preview).parent.mkdir(parents=True, exist_ok=True)
        Path(args.email_mime_preview).write_bytes(mime_msg.as_bytes())
    if args.send:
        to_addr = args.to or secrets.get('GMAIL_USER')
        if not to_addr:
            raise RuntimeError('missing recipient; pass --to or set GMAIL_USER')
        sent = send_gmail(secrets, to_addr, subject, html_body, md, inline_images)
    print(json.dumps({
        'ok': True,
        'sent': bool(sent),
        ...
        'inline_images_prepared': len(inline_images),
        'email_mime_preview': args.email_mime_preview,
    }, ensure_ascii=False))
