#!/usr/bin/env python3
"""LK GMC P2A category/product_type executor.

Dry-run by default. When explicitly approved, patches only
`productAttributes.googleProductCategory` and `productAttributes.productTypes`
via Merchant API v1 ProductInputs for dataSource 10636492695.
"""
...
PREVIEW_CSV = ROOT / 'reports/lk-gmc-2026-05-13-p2-seo-category-readonly-preview.csv'
RUN_STAMP = '2026-05-13-p2-category-product-type-executor'
...
APPROVAL_TEXT_REQUIRED = 'Lucas approved GMC P2A category/product_type pilot apply'
...
ALLOWED_GPCS = {
    ...
    'Luggage & Bags > Handbags, Wallets & Cases',
}
...
def product_input_name(mid: str, pid: str) -> tuple[str, str, str, str, str]:
    ...
    if channel != 'online':
        raise ValueError(f'only_online_supported_for_p2a: {pid}')
...
def patch_category_product_types(mid: str, token: str, pid: str, gpc: str, product_types: list[str]) -> dict[str, Any]:
    ...
    qs = urllib.parse.urlencode({'dataSource': data_source, 'updateMask': 'productAttributes.googleProductCategory,productAttributes.productTypes'})
...
def load_candidates(limit: int) -> list[dict[str, Any]]:
    ...
    with PREVIEW_CSV.open(encoding='utf-8') as f:
        for r in csv.DictReader(f):
            states=json.loads(r.get('decision_states') or '[]')
            gpc=r.get('suggested_googleProductCategory') or ''
            pt=r.get('suggested_productType') or ''
            if 'candidate_p2a_category_product_type' not in states: continue
            if gpc not in ALLOWED_GPCS or not pt: continue
            rows.append({
                ...
                'brand': r['brand'],
                'link': r['link'],
                'current_googleProductCategory': r.get('googleProductCategory_current') or '',
                'current_productTypes': json.loads(r.get('productTypes_current') or '[]'),
                'suggested_googleProductCategory': gpc,
                'suggested_productTypes': [pt],
                'evidence': r.get('category_evidence'),
                'selected_for_apply': False,
            })
    priority = {'Apparel & Accessories > Shoes': 0, 'Apparel & Accessories > Clothing > Shirts & Tops': 1}
    rows.sort(key=lambda r: (priority.get(r['suggested_googleProductCategory'], 9), r['title'], r['product_id']))
    for r in rows[:limit]: r['selected_for_apply'] = True
    return rows
...
def snapshot(records: list[dict[str, Any]], limit: int) -> pathlib.Path:
    ...
    path.write_text(json.dumps({
        ...
        'scope': f'P2A pilot Merchant API v1 ProductInputs patch only; dataSource={DATA_SOURCE_ID}; limit={limit}',
        'approval_text_required': APPROVAL_TEXT_REQUIRED,
        'rollback_instruction': 'Use Merchant API v1 ProductInputs PATCH with previous googleProductCategory/productTypes from current_content_api_product_resource if rollback is needed.',
        'records': records,
...
def status_recheck(w, mid: str, token: str) -> dict[str, Any]:
    ...
    issue_counts = Counter(); required_attr_rows = 0; required_attr_counts = Counter()
    for st in statuses:
        for issue in st.get('itemLevelIssues') or []:
            issue_counts[issue.get('code') or 'unknown'] += 1
        attrs = w.required_attrs(st)
        if attrs:
            required_attr_rows += 1
            for a in attrs: required_attr_counts[a] += 1
    return {
        'fresh_productstatuses_after': len(statuses),
        'required_attr_rows_after': required_attr_rows,
        'required_attr_counts_after': dict(required_attr_counts.most_common()),
        'top_item_level_issue_counts_after': dict(issue_counts.most_common(12)),
    }

def write_outputs(mode: str, rows: list[dict[str, Any]], exec_results: list[dict[str, Any]], verified: list[dict[str, Any]], rollback: pathlib.Path | None, limit: int, post: dict[str, Any] | None):
    ec=Counter(x.get('execution_status') for x in exec_results); vc=Counter(x.get('verify_status') for x in verified)
    selected=[r for r in rows if r['selected_for_apply']]
    bucket_counts = {f'{gpc} / {pts[0] if pts else ""}': cnt for (gpc, pts), cnt in Counter((r['suggested_googleProductCategory'], tuple(r['suggested_productTypes'])) for r in selected).most_common()}
    payload={
        'generated_at': utc_now(),
        'status': 'p2a_category_product_type_apply_verified' if mode=='apply' else 'p2a_category_product_type_dry_run_ready',
        'mode': mode,
        ...
        'summary': {'candidate_total': len(rows), 'selected_for_apply': len(selected), 'limit': limit, 'selected_bucket_counts': bucket_counts, 'execution_results_summary': dict(ec), 'verify_results_summary': dict(vc), 'verified_match_expected': sum(1 for x in verified if x.get('match_expected')), **(post or {})},
        'private_rollback_snapshot_path': str(rollback) if rollback else None,
        'rows': rows,
        'execution_results': exec_results,
        'verified_results': verified,
        'not_performed': ['title_update', 'price_update', 'availability_update', 'merchant_delete', 'shopify_write', 'tiny_write', 'database_write', 'feed_fetch_or_upload', 'message_or_campaign_send'],
    }
    PUBLIC_JSON.write_text(json.dumps(payload, ensure_ascii=False, indent=2)+'\n', encoding='utf-8')
    fields=['product_id','offer_id','title','brand','current_googleProductCategory','current_productTypes','suggested_googleProductCategory','suggested_productTypes','evidence','selected_for_apply']
    ...
    lines=['# LK GMC P2A Category/Product Type Executor — 2026-05-13','',f'Status: `{payload["status"]}`','', '## Escopo','- API: Merchant API v1 ProductInputs PATCH.','- Campos: `productAttributes.googleProductCategory` + `productAttributes.productTypes` somente.',f'- Data source: `accounts/*/dataSources/{DATA_SOURCE_ID}`','- Sem alterações de título, preço, disponibilidade ou produto Shopify.','', '## Resultado',f'- Candidatos totais: {len(rows)}',f'- Selecionados no piloto: {len(selected)}',f'- Buckets selecionados: {payload["summary"]["selected_bucket_counts"]}',f'- Execution: {dict(ec)}',f'- Verify: {dict(vc)}',f'- Match esperado: {payload["summary"]["verified_match_expected"]}/{len(verified)}']
    if post:
        lines += [f'- Productstatuses fresh after: {post["fresh_productstatuses_after"]}', f'- Required rows after: {post["required_attr_rows_after"]}', f'- Required counts after: {post["required_attr_counts_after"]}', f'- Top issues after: {post["top_item_level_issue_counts_after"]}']
...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--apply', action='store_true'); ap.add_argument('--approval-text', default=''); ap.add_argument('--limit', type=int, default=100); ap.add_argument('--verify-delay', type=int, default=90); ap.add_argument('--post-status-delay', type=int, default=120)
    ...
    rows=load_candidates(args.limit); exec_results=[]; verified=[]; rollback=None; post=None
    if args.apply:
        ...
        with progress.open('w', encoding='utf-8') as f:
            ...
            for rec in records:
                ...
                try:
                    resp=patch_category_product_types(mid, token, r['product_id'], r['suggested_googleProductCategory'], r['suggested_productTypes'])
                    item={'product_id': r['product_id'], 'execution_status': 'patched_p2a_category_product_type_v1', 'product_input': resp.get('name')}
                except Exception as e:
        ...
        for res in exec_results:
            ...
            if res.get('execution_status')!='patched_p2a_category_product_type_v1' or not r:
            ...
            try:
                ...
                verified.append({**res, 'verify_status': 'verified_merchant_product_get', 'actual_googleProductCategory': attrs.get('googleProductCategory'), 'actual_productTypes': attrs.get('productTypes') or [], 'match_expected': match})
            except Exception as e:
    ...
    write_outputs(mode, rows, exec_results, verified, rollback, args.limit, post)
