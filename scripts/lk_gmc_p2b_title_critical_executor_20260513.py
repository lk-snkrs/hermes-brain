#!/usr/bin/env python3
"""LK GMC P2B critical title executor.

Patches only productAttributes.title on Merchant API v1 ProductInputs for the
35 rows where GMC title degraded to size/color only. Does not change Shopify.
Dry-run by default; apply requires exact approval text.
"""
...
SOURCE_PREVIEW = ROOT / 'reports/lk-gmc-2026-05-13-p2-seo-category-readonly-preview.csv'
TITLE_PREVIEW = ROOT / 'reports/lk-gmc-2026-05-13-p2b-critical-title-preview-final.csv'
RUN_STAMP = '2026-05-13-p2b-critical-title-executor'
...
APPROVAL_TEXT_REQUIRED = 'Lucas approved GMC P2B critical title apply only on GMC ProductInput with rollback and verification'
...
def product_input_name(mid: str, pid: str) -> tuple[str, str, str, str, str]:
    ...
    if channel != 'online':
        raise ValueError(f'only_online_supported_for_p2b_title: {pid}')
...
def load_rows(limit: int) -> list[dict[str, Any]]:
    source_by_offer = {}
    with SOURCE_PREVIEW.open(encoding='utf-8') as f:
        for r in csv.DictReader(f):
            source_by_offer[r['offer_id']] = r
    rows = []
    with TITLE_PREVIEW.open(encoding='utf-8') as f:
        for r in csv.DictReader(f):
            src = source_by_offer.get(r['offer_id'])
            if not src:
                raise RuntimeError('missing_source_preview_for_offer: ' + r['offer_id'])
            title = (r.get('proposed_gmc_title') or '').strip()
            if not title or len(title) > 150:
                raise RuntimeError(f'invalid_title_for_offer {r["offer_id"]}: length={len(title)}')
            rows.append({
                ...
                'source': r.get('shopify_product_title_source') or '',
                'link': r.get('link') or src.get('link') or '',
...
def snapshot(records: list[dict[str, Any]], limit: int) -> pathlib.Path:
    ...
    path.write_text(json.dumps({
        ...
        'scope': f'P2B critical titles Merchant API v1 ProductInputs patch only; dataSource={DATA_SOURCE_ID}; limit={limit}',
...
def write_outputs(mode: str, rows: list[dict[str, Any]], exec_results: list[dict[str, Any]], verified: list[dict[str, Any]], rollback: pathlib.Path | None, limit: int):
    ...
    payload={
        'generated_at': utc_now(),
        'status': 'p2b_critical_title_apply_verified' if mode=='apply' else 'p2b_critical_title_dry_run_ready',
        'mode': mode,
    ...
    fields=['product_id','offer_id','current_gmc_title','proposed_gmc_title','shopify_product_title','confidence','source','link','selected_for_apply']
...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--apply', action='store_true'); ap.add_argument('--approval-text', default=''); ap.add_argument('--limit', type=int, default=35); ap.add_argument('--verify-delay', type=int, default=90)
    ...
    if args.apply:
        ...
        with progress.open('w', encoding='utf-8') as f:
            ...
            for rec in records:
                ...
                try:
                    ...
                    item={'product_id': r['product_id'], 'offer_id': r['offer_id'], 'execution_status': 'patched_p2b_title_v1', 'product_input': resp.get('name')}
                except Exception as e:
        ...
        for res in exec_results:
            ...
            if res.get('execution_status')!='patched_p2b_title_v1' or not r:
