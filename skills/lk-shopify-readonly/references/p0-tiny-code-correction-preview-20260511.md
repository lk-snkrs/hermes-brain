...
- Tiny requires `sequencia` inside the `produto` record in the root wrapper; `produto.obter` may not return it for child variations, so use `sequencia: "1"` for single-record child-variation writes.
- For apparel child variations whose Tiny `nome` uses hyphen display (`S-P`, `L-G`) while `grade` stores slash (`S/P`, `L/G`), including `grade` in `produto.alterar` can normalize the product name to slash and violate “codigo only”. For apparel retry, send `id`, `sequencia`, `codigo`, and preserved scalar fields (`nome`, `unidade`, `preco`, `preco_promocional`, `origem`, `situacao`, `tipo`) but omit `grade`; verify no non-code changes.
- For Tiny writes, throttle conservatively (e.g. 4–5 seconds between calls, plus cooldown after blocks) to avoid `API Bloqueada - Excedido o número de acessos`.
