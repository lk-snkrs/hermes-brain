# Zipper Gallery — Memória

> LEGACY / SUPERSEDED — este arquivo contém referências históricas migradas de fases anteriores. Não usar paths `/root`, Mem0, OpenClaw/cerebro-cimino ou scripts citados aqui como instrução operacional viva. Fonte atual: `/opt/data/hermes_bruno_ingest/hermes-brain` e rotinas governadas em `areas/operacoes/`.


## Contexto
- **Dono:** Lucas Cimino + Osmar (sócio/diretor comercial)
- **Meta 2026:** R$ 1.400.000
- **Resultado março 2026:** R$ 679.000 (mês excepcional)
- **Galeria de arte contemporânea**

## Equipe
| Nome | Função |
|------|--------|
| Osmar | Diretor comercial + sócio |
| Helo | Comunicação (vídeos) |
| Biz | Comunicação (texto) |
| Mie | Comunicação (fotos/design) |
| Cibele | Financeiro e RH |
| Panda | Produção |

## Canais
| Canal | Contato |
|-------|---------|
| Instagram | @zippergaleria (120K+) |
| WhatsApp | 551143064306 (instância ZipperGaleria) |
| Email | lucas@zippergaleria.com.br |
| Email produção | producao@zippergaleria.com.br |

## Bancos de Dados
- **Zipper Vendas:** `pcstqxpdzibheuopjkas` — 2.058 vendas reais de obras
- **SPITI/CRM:** `rmdugdkantdydivgnimb` — leilão SPITI

## Feiras 2026
| Feira | Data | Status |
|-------|------|--------|
| SP-Arte | Abril 2026 | Urgente — D-30 |
| ArPa | TBD | Planeamento Q2-Q4 |
| ArtRio | TBD | Planeamento Q2-Q4 |
| Farol Santander | TBD | Planeamento Q2-Q4 |

## Programa Zip'Up
- 6 artistas no programa

## Como Falo (TOM Zipper)
- **Tom:** Leve e descontraído, nunca excessivo, arte como paixão
- **Artigo:** "a" obra, "o" Picasso (mas "a" Andy Warhol)
- **Jamais:** "百分之百", "INCŔÍVEL", Tom de liquidação
- **Sempre:** Curadoria, pertencimento, identidade cultural
- **Estrangeiros:** Usar nome original em itálico (Gagosian, not Gagossian)

## Decisões Permanentes
- **Vendas reais** → banco Zipper Vendas (`vendas_tango`)
- **Leilão SPITI** → banco SPITI (`spiti_lotes`) — nunca confundir
- **Tom:** leve e descontraído, nunca excessivo

## Lições Aprendidas
- Março 2026 foi excepcional (R$ 679k) — não usar como referência para projeções
- Sempre verificar `pedido_origem` no banco para entender canal de venda
- Dois bancos distintos — nunca confundir vendas reais vs leilão

## Fonte
Sincronizado do cerebro-cimino (VPS: /root/cerebro-cimino) para hermes-brain (/root/hermes-brain)
